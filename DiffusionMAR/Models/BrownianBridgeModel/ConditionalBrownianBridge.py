...
class ConditionalBrownianBridgeModel(nn.Module):
    ...
    def p_losses(self, x0, y, cond, t):
        ...
        comb = torch.cat((x_t, cond), dim = 1)
        
        dif_prediction = self.denoise_fn(comb, timesteps=t)
    ...
    @torch.no_grad()
    def p_sample(self, x_t, y, cond, i, clip_denoised=False):
    ...
    @torch.no_grad()
    def p_sample_loop(self, y, cond, clip_denoised=False):
        imgs, one_step_imgs = [y], []
        for i in tqdm(range(len(self.sample_schedule))):
            img, x0_recon = self.p_sample(x_t = imgs[-1], y = y, cond = cond, i = i, clip_denoised = clip_denoised)
            imgs.append(img)
